...
def add_transfer_item(transfer_id):
    ...
    try:
        ...
        from_warehouse_code = request.form.get('from_warehouse_code', '').strip()
        to_warehouse_code = request.form.get('to_warehouse_code', '').strip()
        from_bin = request.form.get('from_bin', '').strip()
        to_bin = request.form.get('to_bin', '').strip()
        batch_number_raw = request.form.get('batch_number', '').strip()
        
        # Normalize batch number for consistent comparison (trim whitespace, normalize case for comparison)
        ...
        # **ENHANCED DUPLICATE PREVENTION LOGIC FOR INVENTORY TRANSFERS**
        # Check if this item_code + batch_number combination already exists in this transfer
        # This allows the same item to be transferred multiple times with different batch numbers
        # Uses normalized (trimmed + uppercase) batch comparison for consistency
        existing_items_for_code = InventoryTransferItem.query.filter_by(
            transfer_id=transfer_id,
            item_code=item_code
        ).all()
        
        # If batch_number is provided, check for exact item_code + batch_number combination (normalized)
        if batch_number_normalized:
            for existing_item in existing_items_for_code:
                ...
                if existing_batch == batch_number_normalized:
                    flash(f'Item {item_code} with batch number "{batch_number}" has already been added to this inventory transfer. The same item with the same batch can only be transferred once per transfer request.', 'error')
                    return redirect(url_for('inventory_transfer.detail', transfer_id=transfer_id))
        else:
            # For items without batch numbers, check for existing items with no batch
            # This only applies to non-batch-managed items
            for existing_item in existing_items_for_code:
                existing_batch = (existing_item.batch_number or '').strip()
                if not existing_batch:
                    flash(f'Item {item_code} (without batch) has already been added to this inventory transfer. Each item without batch can only be transferred once per transfer request.', 'error')
                    return redirect(url_for('inventory_transfer.detail', transfer_id=transfer_id))
